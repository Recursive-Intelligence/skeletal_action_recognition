...
from pose_estimation.lightweight_open_pose.utilities import draw
import argparse
...
def preds2label(confidence):
    ...
    labels = {
        NTU60_CLASSES[int(class_inds[j])]: float(class_scores[j].item())
        for j in range(k)
    }
    return labels
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--device", help="Device to use (cpu, cuda)", type=str, default="cpu"
    )
    parser.add_argument(
        "--accelerate",
    ...
    parser.add_argument(
        ...
        type=str,
        default="stgcn_ntu_cv_lw_openpose",
    ...
    model_saved_path = action_classifier.download(
        path=os.path.join("./pretrained_models", args.method),
        method_name=args.method,
        mode="pretrained",

...
class LightweightOpenPoseLearner(Learner):
    def __init__(
        ...
        super(LightweightOpenPoseLearner, self).__init__(
            lr=lr,
        ...
        self.parent_dir = (
            temp_path  # Parent dir should be filled by the user according to README
        )

        self.num_refinement_stages = (
            num_refinement_stages  # How many extra refinement stages to add
        )
        ...
        if self.backbone not in supportedBackbones:
            raise ValueError(
                self.backbone
                + " not a valid backbone. Supported backbones:"
                + str(supportedBackbones)
            )
        ...
        self.output_name = os.path.join(
            self.parent_dir, output_name
        )  # Path to json file containing detections
        self.visualize = visualize
        self.base_height = base_height
        if scales is None:
            scales = [1]
        self.multiscale = (
            multiscale  # If set to true, overwrites self.scales to [0.5, 1.0, 1.5, 2.0]
        )
        self.scales = scales
        if self.use_stride:
            self.stride = 8 * 2
    ...
    def fit(
        ...
        val_images_folder_name="val2017",
        val_annotations_filename="person_keypoints_val2017.json",
    ):
        """
        ...
        if self.checkpoint_load_iter == 0:
            ...
            if self.backbone == "mobilenet":
                backbone_weights_path = os.path.join(
                    self.parent_dir, "mobilenet_sgd_68.848.pth.tar"
                )
            elif self.backbone == "mobilenetv2":
                backbone_weights_path = os.path.join(
                    self.parent_dir, "mobilenetv2_1.0-f2a8633.pth.tar"
                )
            elif self.backbone == "shufflenet":
                backbone_weights_path = os.path.join(
                    self.parent_dir, "shufflenet.pth.tar"
                )
            try:
                checkpoint = torch.load(
                    backbone_weights_path, map_location=torch.device(self.device)
                )
            except FileNotFoundError as e:
                e.strerror = (
                    "Pretrained weights 'pth.tar' file must be placed in temp_path provided. \n "
            ...
            checkpoint_name = (
                "checkpoint_iter_" + str(self.checkpoint_load_iter) + ".pth"
            )
            checkpoints_folder = os.path.join(
                self.parent_dir, "{}_checkpoints".format(self.experiment_name)
            )
            ...
            try:
                checkpoint = torch.load(
                    full_path, map_location=torch.device(self.device)
                )
            except FileNotFoundError as e:
                e.strerror = (
                    "File " + checkpoint_name + " not found inside checkpoints_folder, "
        ...
        optimizer = optim.Adam(
            [
                ...
                {
                    "params": get_parameters_conv_depthwise(self.model.model, "weight"),
                    "weight_decay": 0,
                },
                {
                    "params": get_parameters_bn(self.model.model, "weight"),
                    "weight_decay": 0,
                },
                {
                    "params": get_parameters_bn(self.model.model, "bias"),
                    "lr": self.lr * 2,
                    "weight_decay": 0,
                },
                {
                    "params": get_parameters_conv(self.model.cpm, "weight"),
                    "lr": self.lr,
                },
                {
                    "params": get_parameters_conv(self.model.cpm, "bias"),
                    "lr": self.lr * 2,
                    "weight_decay": 0,
                },
                {
                    "params": get_parameters_conv_depthwise(self.model.cpm, "weight"),
                    "weight_decay": 0,
                },
                {
                    "params": get_parameters_conv(self.model.initial_stage, "weight"),
                    "lr": self.lr,
                },
                {
                    "params": get_parameters_conv(self.model.initial_stage, "bias"),
                    "lr": self.lr * 2,
                    "weight_decay": 0,
                },
                {
                    "params": get_parameters_conv(
                        self.model.refinement_stages, "weight"
                    ),
                    "lr": self.lr * 4,
                },
                {
                    "params": get_parameters_conv(self.model.refinement_stages, "bias"),
                    "lr": self.lr * 8,
                    "weight_decay": 0,
                },
                {
                    "params": get_parameters_bn(self.model.refinement_stages, "weight"),
                    "weight_decay": 0,
                },
                {
                    "params": get_parameters_bn(self.model.refinement_stages, "bias"),
                    "lr": self.lr * 2,
                    "weight_decay": 0,
                },
            ],
        ...
        if self.lr_schedule != "":
            ...
            scheduler = optim.lr_scheduler.MultiStepLR(
                optimizer, milestones=drop_after_epoch, gamma=0.333
            )
            ...
            for batch_data in train_loader:
                ...
                for loss_idx in range(len(total_losses) // 2):
                    ...
                    total_losses[loss_idx * 2] += (
                        losses[-2].item() / self.batches_per_iter
                    )
                    total_losses[loss_idx * 2 + 1] += (
                        losses[-1].item() / self.batches_per_iter
                    )
                    ...
                    if logging:
                        for loss_idx in range(len(total_losses) // 2):
                            file_writer.add_scalar(
                                tag="stage" + str(loss_idx + 1) + "_paf_loss",
                                scalar_value=total_losses[loss_idx * 2 + 1]
                                / self.log_after,
                                global_step=num_iter,
                            )
                            file_writer.add_scalar(
                                tag="stage" + str(loss_idx + 1) + "_heatmaps_loss",
                                scalar_value=total_losses[loss_idx * 2]
                                / self.log_after,
                                global_step=num_iter,
                            )
                    if not silent and verbose:
                        ...
                        for loss_idx in range(len(total_losses) // 2):
                            print(
                                "\n".join(
                                    ...
                                    loss_idx + 1,
                                    total_losses[loss_idx * 2 + 1] / self.log_after,
                                    loss_idx + 1,
                                    total_losses[loss_idx * 2] / self.log_after,
                                )
                            )
                ...
                if (
                    ...
                    and num_iter % self.checkpoint_after_iter == 0
                ):
                ...
                if (
                    ...
                    if logging:
                        file_writer.add_scalar(
                            tag="Average Precision @IoU=0.5:0.95, area = all",
                            scalar_value=eval_results["average_precision"][0],
                            global_step=num_iter,
                        )
                        file_writer.add_scalar(
                            tag="Average Precision @IoU=0.5, area = all",
                            scalar_value=eval_results["average_precision"][1],
                            global_step=num_iter,
                        )
                        file_writer.add_scalar(
                            tag="Average Precision @IoU=0.75, area = all",
                            scalar_value=eval_results["average_precision"][2],
                            global_step=num_iter,
                        )
                        file_writer.add_scalar(
                            tag="Average Precision @IoU=0.5:0.95, area = medium",
                            scalar_value=eval_results["average_precision"][3],
                            global_step=num_iter,
                        )
                        file_writer.add_scalar(
                            tag="Average Precision @IoU=0.5:0.95, area = large",
                            scalar_value=eval_results["average_precision"][4],
                            global_step=num_iter,
                        )
                        file_writer.add_scalar(
                            tag="Average Recall @IoU=0.5:0.95, area = all",
                            scalar_value=eval_results["average_precision"][0],
                            global_step=num_iter,
                        )
                        file_writer.add_scalar(
                            tag="Average Recall @IoU=0.5, area = all",
                            scalar_value=eval_results["average_precision"][1],
                            global_step=num_iter,
                        )
                        file_writer.add_scalar(
                            tag="Average Recall @IoU=0.75, area = all",
                            scalar_value=eval_results["average_precision"][2],
                            global_step=num_iter,
                        )
                        file_writer.add_scalar(
                            tag="Average Recall @IoU=0.5:0.95, area = medium",
                            scalar_value=eval_results["average_precision"][3],
                            global_step=num_iter,
                        )
                        file_writer.add_scalar(
                            tag="Average Recall @IoU=0.5:0.95, area = large",
                            scalar_value=eval_results["average_precision"][4],
                            global_step=num_iter,
                        )
                        avg_precision = np.mean(eval_results["average_precision"])
                        file_writer.add_scalar(
                            tag="Average Precision - all",
                            scalar_value=avg_precision,
                            global_step=num_iter,
                        )
                        avg_recall = np.mean(eval_results["average_recall"])
                        file_writer.add_scalar(
                            tag="Average Recall - all",
                            scalar_value=avg_recall,
                            global_step=num_iter,
                        )
                        file_writer.add_scalar(
                            tag="Average Score - all",
        ...
        return {
            "paf_losses": paf_losses,
            "heatmap_losses": heatmap_losses,
    ...
    def eval(
        ...
        data = self.__prepare_val_dataset(
            dataset,
        ...
        if self.model is None and self.checkpoint_load_iter != 0:
            ...
            raise AttributeError(
                "self.model is None. Please load a model or set checkpoint_load_iter."
            )
        ...
        if len(coco_result) != 0:
            if use_subset:
                result = run_coco_eval(
                    os.path.join(dataset.path, "val_subset.json"),
                    self.output_name,
                    verbose=not silent,
                )
            else:
                result = run_coco_eval(
                    os.path.join(dataset.path, annotations_filename),
                    self.output_name,
                    verbose=not silent,
                )
            return {
                "average_precision": result.stats[0:5],
                "average_recall": result.stats[5:],
            }
        else:
            if not silent and verbose:
                print("Evaluation ended with no detections.")
            return {
                "average_precision": [0.0 for _ in range(5)],
                "average_recall": [0.0 for _ in range(5)],
            }
    ...
    def infer(self, img, upsample_ratio=4, track=True, smooth=True):
        ...
        if self.ort_session is not None:
            stages_output = self.ort_session.run(
        ...
        for kpt_id in range(all_keypoints.shape[0]):
            all_keypoints[kpt_id, 0] = (
                all_keypoints[kpt_id, 0] * self.stride / upsample_ratio - pad[1]
            ) / scale
            all_keypoints[kpt_id, 1] = (
                all_keypoints[kpt_id, 1] * self.stride / upsample_ratio - pad[0]
            ) / scale
        current_poses = []
    ...
    def save(self, path, verbose=False):
        ...
        if self.ort_session is None:
            ...
            torch.save(
                custom_dict,
                os.path.join(
                    full_path_to_model_folder, model_metadata["model_paths"][0]
                ),
            )
            if verbose:
        ...
        with open(
            os.path.join(full_path_to_model_folder, folder_name_no_ext + ".json"), "w"
        ) as outfile:
        ...
        if self.model is None:
            ...
            if self.backbone == "mobilenet":
                self.model = PoseEstimationWithMobileNet(
                    self.num_refinement_stages, use_stride=self.use_stride
                )
            elif self.backbone == "mobilenetv2":
                self.model = PoseEstimationWithMobileNetV2(
                    self.num_refinement_stages, width_mult=self.mobilenetv2_width
                )
            elif self.backbone == "shufflenet":
                self.model = PoseEstimationWithShuffleNet(
                    self.num_refinement_stages, groups=self.shufflenet_groups
                )
        else:
            raise UserWarning(
                "Tried to initialize model while model is already initialized."
            )
    ...
    def count_parameters(self):
        ...
        if self.model is None:
            raise UserWarning(
                "Model is not initialized, can't count trainable parameters."
            )
        ...
        if mode == "pretrained":
            ...
            if self.backbone == "mobilenet":
                if not os.path.exists(os.path.join(path, "openpose_default.json")):
                    file_url = os.path.join(
                        url, "openpose_default/openpose_default.json"
                    )
                ...
                if not os.path.exists(os.path.join(path, "openpose_default.pth")):
                    file_url = os.path.join(
                        url, "openpose_default/openpose_default.pth"
                    )
            ...
            if self.backbone == "mobilenet":
                if not os.path.exists(
                    os.path.join(self.temp_path, "mobilenet_sgd_68.848.pth.tar")
                ):
                ...
                if not os.path.exists(
                    os.path.join(self.temp_path, "mobilenetv2_1.0-f2a8633.pth.tar")
                ):
                ...
                if not os.path.exists(
                    os.path.join(self.temp_path, "shufflenet.pth.tar")
                ):
                    file_url = os.path.join(url, "shufflenet.pth.tar")
                    urlretrieve(
                        file_url, os.path.join(self.temp_path, "shufflenet.pth.tar")
                    )
                    if verbose:
            ...
            urlretrieve(
                file_url, os.path.join(self.temp_path, "dataset", "annotation.json")
            )
            # Download test image
            file_url = os.path.join(url, "dataset", "image", "000000000785.jpg")
            urlretrieve(
                file_url,
                os.path.join(self.temp_path, "dataset", "image", "000000000785.jpg"),
            )
    ...
    def __infer_eval(self, img):
        ...
        for ratio in scales_ratios:
            ...
            heatmaps = heatmaps[
                pad[0] : heatmaps.shape[0] - pad[2],
                pad[1] : heatmaps.shape[1] - pad[3] :,
                :,
            ]
            heatmaps = cv2.resize(
                heatmaps, (width, height), interpolation=cv2.INTER_CUBIC
            )
            ...
            if self.half:
                pafs = np.float32(pafs)
            pafs = cv2.resize(
                pafs, (0, 0), fx=stride, fy=stride, interpolation=cv2.INTER_CUBIC
            )
            pafs = pafs[
                pad[0] : pafs.shape[0] - pad[2], pad[1] : pafs.shape[1] - pad[3], :
            ]
            ...
            if prepared_annotations_name not in f:
                if verbose:
                    print(
                        ...
                        + " in dataset.path, creating new..."
                    )
                prepare_train_labels.convert_annotations(
                    annotations_file,
                    output_path=os.path.join(dataset.path, prepared_annotations_name),
                )
                if verbose:
                    print(
                        "Created new .pkl file containing prepared annotations in internal format."
                    )
            prepared_train_labels = os.path.join(
                dataset.path, prepared_annotations_name
            )

            sigma = 7
            paf_thickness = 1
            return CocoTrainDataset(
        ...
        if isinstance(dataset, ExternalDataset):
            ...
            if use_subset:
                ...
                if subset_name not in f:
                    ...
                    make_val_subset.make_val_subset(
                        val_labels_file,

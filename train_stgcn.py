...
class SpatioTemporalGCNLearner(Learner):
    def __init__(
        ...
        if self.num_subframes > self.num_frames:
            raise ValueError(
                "number of subframes should be smaller than number of frames."
            )
        ...
        if self.method_name is None or self.method_name not in [
            "stgcn",
            "tagcn",
            "stbln",
        ]:
    ...
    def fit(
        ...
        if self.logging_path != "" and self.logging_path is not None:
            self.logging = True
            self.tensorboard_logging_path = os.path.join(
                self.logging_path, self.experiment_name + "_tensorboard"
            )
            if self.model_train_state:
                self.train_writer = SummaryWriter(
                    os.path.join(self.tensorboard_logging_path, "train"), "train"
                )
                self.val_writer = SummaryWriter(
                    os.path.join(self.tensorboard_logging_path, "val"), "val"
                )
            else:
                self.val_writer = SummaryWriter(
                    os.path.join(self.tensorboard_logging_path, "test"), "test"
                )
        else:
            self.logging = False
        ...
        if self.checkpoint_load_iter != 0:
            ...
            checkpoint_name = (
                ...
                + str(self.epochs - 1)
                + "-"
                + str(int(self.checkpoint_load_iter))
                + ".pt"
            )
        ...
        if self.optimizer_name == "sgd":
            ...
            self.optimizer_ = optim.Adam(
                self.model.parameters(), lr=self.base_lr, weight_decay=weight_decay
            )
        else:
            raise ValueError(
                self.optimizer_
                + "is not a valid optimizer name. Supported optimizers: sgd, adam"
            )
        ...
        if self.lr_schedule != "":
            ...
            scheduler = optim.lr_scheduler.MultiStepLR(
                self.optimizer_,
                milestones=self.drop_after_epoch,
                gamma=0.1,
                last_epoch=-1,
                verbose=True,
            )
        ...
        if verbose:
            print(
                "best accuracy: ", self.best_acc, " model_name: ", self.experiment_name
            )
        return {
    ...
    def eval(
        ...
        for k in show_topk:
            self.__print_log(
                "\tTop{}: {:.2f}%".format(k, 100 * val_loader.dataset.top_k(score, k))
            )
        if save_score and self.logging:
            with open(
                "{}/epoch{}_{}_score.pkl".format(self.logging_path, epoch + 1, "val"),
                "wb",
            ) as f:
    ...
    def init_model(self, old_model = False):
        ...
        if self.method_name == "stgcn":
            ...
            self.model = TAGCN(
                ...
                num_selected_frames=self.num_subframes,
                cuda_=cuda_,
            )
            if self.logging:
    ...
    def infer(self, SkeletonSeq_batch):
        ...
        if "cuda" in self.device:
            SkeletonSeq_batch = Variable(
                SkeletonSeq_batch.float().cuda(self.output_device), requires_grad=False
            )
        else:
            SkeletonSeq_batch = Variable(SkeletonSeq_batch.float(), requires_grad=False)
        if self.ort_session is not None:
            output = self.ort_session.run(
                None, {"data": np.array(SkeletonSeq_batch.cpu())}
            )
        else:
            if self.model is None:
                raise UserWarning(
    ...
    def optimize(self, do_constant_folding=True):
        ...
        try:
            self.__convert_to_onnx(
                os.path.join(self.parent_dir, self.experiment_name, "onnx_model.onnx"),
                do_constant_folding,
            )
        except FileNotFoundError:
            # Create temp directory
            os.makedirs(
                os.path.join(self.parent_dir, self.experiment_name), exist_ok=True
            )
            self.__convert_to_onnx(
                os.path.join(self.parent_dir, self.experiment_name, "onnx_model.onnx"),
                do_constant_folding,
    ...
    def __convert_to_onnx(self, output_name, do_constant_folding=False, verbose=False):
        ...
        torch.onnx.export(
            ...
            dynamic_axes={
                "onnx_input": {0: "n"},  # variable lenght axes
                "onnx_output": {0: "n"},
            },
        )
    ...
    def __load_from_pt(self, path, verbose=True):
        ...
        if path is not None:
            ...
            try:
                ...
                e.strerror = (
                    "Pretrained weights '.pt' file must be placed in path provided. \n "
            ...
            if "cuda" in self.device:
                weights = OrderedDict(
                    [
                        [k.split("module.")[-1], v.cuda(self.output_device)]
    ...
    def multi_stream_eval(
        ...
        if self.logging:
            self.__print_log(
                "\tMulti_Stream_Top{}: {:.2f}%".format(1, 100 * accuracy_top1)
            )
            self.__print_log(
                "\tMulti_Stream_Top{}: {:.2f}%".format(5, 100 * accuracy_top5)
            )
    ...
    def download(
        ...
        if not os.path.exists(
            os.path.join(path, "{}_checkpoints".format(self.experiment_name), file_name)
        ):
            os.makedirs(
                os.path.join(
                    path, "{}_checkpoints".format(self.experiment_name), file_name
                )
            )
        ...
        if mode == "pretrained":
            ...
            if not os.path.exists(
                ...
                file_url = os.path.join(
                    url, "pretrained_models", method_name, file_name, file_name + ".pt"
                )
                urlretrieve(
                    file_url,
                    os.path.join(
            ...
            downloaded_files_path = os.path.join(
                path, "{}_checkpoints".format(self.experiment_name), file_name
            )
            ...
            if not os.path.exists(
                ...
                # Download train data
                file_url = os.path.join(
                    url, "data", self.dataset_name, "train_joints.npy"
                )
                urlretrieve(
                    file_url, os.path.join(path, self.dataset_name, "train_joints.npy")
                )
            else:
                if verbose:
            ...
            if not os.path.exists(
                ...
                file_url = os.path.join(
                    url, "data", self.dataset_name, "train_labels.pkl"
                )
                urlretrieve(
                    file_url, os.path.join(path, self.dataset_name, "train_labels.pkl")
                )
            else:
                if verbose:
            ...
            if not os.path.exists(
                ...
                # Download val data
                file_url = os.path.join(
                    url, "data", self.dataset_name, "val_joints.npy"
                )
                urlretrieve(
                    file_url, os.path.join(path, self.dataset_name, "val_joints.npy")
                )
            else:
                if verbose:
            ...
            if not os.path.exists(
                ...
                file_url = os.path.join(
                    url, "data", self.dataset_name, "val_labels.pkl"
                )
                urlretrieve(
                    file_url, os.path.join(path, self.dataset_name, "val_labels.pkl")
                )
            else:
                if verbose:
            ...
            downloaded_files_path = os.path.join(
                path, self.dataset_name, "val_joints.npy"
            )
